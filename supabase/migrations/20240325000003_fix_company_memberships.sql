...
-- Create company_memberships table with proper foreign key relationships
...
-- Create function to handle user invitation
...
BEGIN
    ...
    IF v_user_id IS NOT NULL THEN
        ...
        SET role = EXCLUDED.role,
            ...
            updated_at = NOW();
    ELSE
        -- If user doesn't exist, create pending membership
        INSERT INTO company_memberships (company_id, user_id, role, status)
        VALUES (p_company_id, NULL, p_role, 'pending')
        ON CONFLICT (company_id, user_id) DO UPDATE
        SET role = EXCLUDED.role,
            status = 'pending',
            updated_at = NOW();
